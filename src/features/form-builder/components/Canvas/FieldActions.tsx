...
const FieldActions: React.FC<FieldActionsProps> = ({ field, isSelected, onDuplicate, onDelete }) => {
  ...
  return (
    <div className={`absolute right-2 top-2 flex gap-1 transition-opacity ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} z-20`}>
      ...
      <button onClick={(e) => onDuplicate(fieldSettings, e)} className="text-text-secondary hover:text-white hover:bg-primary rounded p-1 transition-colors">
        <span className="material-symbols-outlined text-lg">content_copy</span>
      </button>
      <button onClick={(e) => onDelete(fieldSettings.ComponentId, e)} className="text-text-secondary hover:text-red-400 hover:bg-red-400/10 rounded p-1 transition-colors">
